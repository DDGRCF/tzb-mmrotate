...
def crop_and_save_img(info, windows, window_anns, img_dir, no_padding,
    ...
    ds = gdal.Open(osp.join(img_dir, info['filename']))
    patch_infos = []
    for i in range(windows.shape[0]):
        patch_info = dict()
        ...
        ann = window_anns[i]
        if ignore_empty_patch and not len(ann['labels']):
            continue
...
def single_split(arguments, sizes, gaps, img_rate_thr, iof_thr, no_padding,
    ...
    patch_infos = crop_and_save_img(info, windows, window_anns, img_dir,
                                    ...
                                    anno_dir, img_ext, ignore_empty_patch)
    assert patch_infos
...
def response_makedirs(dirname: str, msg: str = None):
...
def main():
    ...
    response_makedirs(save_imgs)
    response_makedirs(save_files)
    ...
    worker = partial(
        ...
        ignore_empty_patch=args.ignore_empty_patch,
        lock=manager.Lock(),
